...
$(document).ready(function(){
    EmojiConfig.init_emoticons();
    EmojiConfig.init_unified();
    const markup = `<div class="card card-primary card-outline card-outline-tabs">
        <div class="card-header p-0 border-bottom-0" style="position: sticky; top: 0; background: inherit;">
            <ul class="nav nav-tabs" id="emoji-tabs" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="emoji-tabs-one-tab" data-toggle="pill" href="#emoji-tabs-one" role="tab" aria-controls="emoji-tabs-one" aria-selected="true">😀</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="emoji-tabs-two-tab" data-toggle="pill" href="#emoji-tabs-two" role="tab" aria-controls="emoji-tabs-two" aria-selected="false">🐶</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="emoji-tabs-three-tab" data-toggle="pill" href="#emoji-tabs-three" role="tab" aria-controls="emoji-tabs-three" aria-selected="false">🔨</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="emoji-tabs-four-tab" data-toggle="pill" href="#emoji-tabs-four" role="tab" aria-controls="emoji-tabs-four" aria-selected="false">✈️</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="emoji-tabs-five-tab" data-toggle="pill" href="#emoji-tabs-five" role="tab" aria-controls="emoji-tabs-five" aria-selected="false">🔢</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="emoji-tabsContent">
                <div class="tab-pane fade emoji-list show active " id="emoji-tabs-one" role="tabpanel" aria-labelledby="emoji-tabs-one-tab">
                ${EmojiConfig.EmojiCategories[0].map(code =>`
                    <a class="emoji-item">${EmojiConfig.Emoji[code][0]}</a>
                    `).join('')}
                </div>
                <div class="tab-pane fade emoji-list" id="emoji-tabs-two" role="tabpanel" aria-labelledby="emoji-tabs-two-tab">
                ${EmojiConfig.EmojiCategories[1].map(code =>`
                    <a class="emoji-item">${EmojiConfig.Emoji[code][0]}</a>
                    `).join('')}
                </div>
                <div class="tab-pane fade emoji-list" id="emoji-tabs-three" role="tabpanel" aria-labelledby="emoji-tabs-three-tab">
                ${EmojiConfig.EmojiCategories[2].map(code =>`
                    <a class="emoji-item">${EmojiConfig.Emoji[code][0]}</a>
                    `).join('')}
                </div>
                <div class="tab-pane fade emoji-list" id="emoji-tabs-four" role="tabpanel" aria-labelledby="emoji-tabs-four-tab">
                ${EmojiConfig.EmojiCategories[3].map(code =>`
                    <a class="emoji-item">${EmojiConfig.Emoji[code][0]}</a>
                    `).join('')}
                </div>
                <div class="tab-pane fade emoji-list" id="emoji-tabs-five" role="tabpanel" aria-labelledby="emoji-tabs-five-tab">
                ${EmojiConfig.EmojiCategories[4].map(code =>`
                    <a class="emoji-item">${EmojiConfig.Emoji[code][0]}</a>
                    `).join('')}
                </div>
            </div>
    ...
    $('.chat-emoji-panel').html(markup);

    var CaretUtil = { };
CaretUtil.setCaretPosition = function(container, position) {
    if(position >= 0) {
        var selection = window.getSelection();
        var range = saved ? saved.range : undefined;
        if(!range || range.toString() === ''){
            range = CaretUtil.createRange(container,{ count : position});
...
CaretUtil.createRange = function(node,chars,range) {
    ...
    var saved = { editor: document.getElementById('chat-input'), pos:0 , range: null};
    $(document).on('selectionchange', function(){
        var sel = document.getSelection();
        if(sel && sel.extentNode && $(sel.extentNode).closest('#chat-input').length > 0){
            var editor = document.getElementById('chat-input');
            saved.editor = editor;
            saved.pos = CaretUtil.getCaretPosition(editor);
            saved.range = sel.getRangeAt(0); 
        }
    });
    $(document).on('click', '.emoji-item', function(){
        var text = $(this).text();
        if (saved && saved.editor) {
            CaretUtil.setCaretPosition(saved.editor, saved.pos)
        }
        webeditor.insertTextAtCursor(text);
        return false;
    });
});
