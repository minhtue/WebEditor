...
(function($, window, document, undefined) {
    ...
    var defaults = {
        contentCallback: function(item, options) {return item[options.contentField] || '' ? item[options.contentField] : item.id;},
        ...
        refAction: 'data-category-action',
        group: 0,
        maxDepth: 5,
        threshold: 20,
        ...
        itemRenderer: function(item_attrs, content, children, options, item) {
            ...
            html += '<a href="javascript:;" class="category-edit btn btn-outline-primary" ' + item_attrs_string + ' ' + options.refAction + '="edit"><i class="fal fa-pencil"></i></a>'
            html += '<a href="javascript:;" class="category-delete  btn btn-outline-danger" ' + item_attrs_string + ' ' + options.refAction + '="delete"><i class="fal fa-trash"></i></a>'
            html += children;
    ...
    Plugin.prototype = {
        ...
        _buildItem: function(item, options) {
            ...
            var content = options.contentCallback(item, options);
        ...
        serialize: function(itemId) {
            var data, list = this, step = function(level) {
                var array = [], items = document.createDocumentFragment();
                if(level === undefined || level.length === 0){
                    return array;
                }
                if(level[0].tagName.toLowerCase() === list.options.itemNodeName){
                    items = level;
                } else {
            ...
            var currentItem = itemId ? this._getItemById(itemId).first() : list.el.find(list.options.listNodeName).first();
            if(!currentItem){
                currentItem = list.el.find(list.options.listNodeName).first();
            }
            data = step(currentItem);
            return data;
        },
